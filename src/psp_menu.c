...
psp_menu_display_save_name()
{
  ...
  snprintf(buffer, 30, "%s", CPC.cpc_save_name);
...
psp_display_screen_menu(void)
{
  ...
  for (menu_id = 0; menu_id < MAX_MENU_ITEM; menu_id++, y += y_step) {
    ...
    if (menu_id == MENU_COMMAND) {
      ...
      sprintf(buffer,": %s", command_name[cur_command]);
 ...
 if (cur_menu_id <= MENU_DEL_SLOT) {
    ...
    if (CPC.cpc_save_state[cur_slot].thumb) {
      psp_sdl_blit_thumb(x, y, CPC.cpc_save_state[cur_slot].surface);
    // } else {
  ...
  while (! end_menu)
  {
    ...
    if ((new_pad == GP2X_CTRL_CIRCLE))
    {
      switch (cur_menu_id )
      {
        ...
        case MENU_COMMAND   : psp_main_menu_command();
                              end_menu = 1;
        break;
