...
static  int sel=0;
  ...
  for(;;)
  {
    ...
    psp_sdl_back2_rectangle(x, y, 170, rows * 10);
    if (display_thumb) {
      psp_sdl_blit_thumb(180, 135, save_surface);
    }
...
static char user_filedir_snap[GP2X_FMGR_MAX_NAME];
...
psp_fmgr_menu(int format, char drive)
{
  ...
  if (first) {
    first = 0;
    strcpy(user_filedir_kbd, CPC.cpc_kbd_path);
    strcpy(user_filedir_set, CPC.cpc_set_path);
    strcpy(user_filedir_cht, CPC.cpc_cht_path);
    strcpy(user_filedir_joy, CPC.cpc_joy_path);
    strcpy(user_filedir_disk, CPC.cpc_disk_path);
    strcpy(user_filedir_snap, CPC.cpc_snap_path);
  }
